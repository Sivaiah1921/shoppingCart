...
const AccessoriesListing = (props) =>{
    
  const {_products} = props._products
  const filterAccessories = _products.filter((value) => value.category === "electronics");
  console.log(filterAccessories, "_products");
          ...
          <div className="product__listingItems">
            {filterAccessories.length === 0 ? (
              <div className="product__listingItems">
                ...
                filterAccessories.map((item, index) => (
                <div
                  key={index}
...
export default connect(mapStateToProps, mapDispatchToProps)(AccessoriesListing);
